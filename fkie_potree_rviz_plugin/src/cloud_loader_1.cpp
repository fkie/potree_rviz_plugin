...
#define LOG_STDERR std::cerr << "[fkie_potree_rviz_plugin::CloudLoader1] "

namespace fkie_potree_rviz_plugin
{

CloudLoader1::CloudLoader1(const std::shared_ptr<CloudMetaData>& meta_data) : meta_data_(meta_data) {}
...
std::size_t CloudLoader1::estimatedPointCount(const std::shared_ptr<PotreeNode>& node) const
{
    fs::path bin_file = fileName(meta_data_, node->name(), ".bin");
    return fs::file_size(bin_file) / meta_data_->point_byte_size_;
}

void CloudLoader1::loadNodeHierarchy(const std::shared_ptr<PotreeNode>& root_node) const
{
    ...
    if (!f.good())
        LOG_STDERR << "failed to read file: " << hrc_file << std::endl;
    f.read(&child_map, 1);
...
void CloudLoader1::loadPoints(const std::shared_ptr<PotreeNode>& node, bool recursive) const
{
    fs::path bin_file = fileName(meta_data_, node->name(), ".bin");
    if (!fs::is_regular_file(bin_file))
    {
        LOG_STDERR << "file not found: " << bin_file << std::endl;
        return;
    }
    ...
    if (!f.good())
    {
        LOG_STDERR << "failed to open file: " << bin_file << std::endl;
        return;
    }
    std::vector<char> data(size);
    if (!f.read(data.data(), size))
    {
        LOG_STDERR << "failed to read file: " << bin_file << std::endl;
        return;
    }
    ...
    if (point_count == 0)
    {
        LOG_STDERR << "empty node: " << node->name() << std::endl;
    ...
    for (const CloudMetaData::PointAttribute& attr : meta_data_->pointAttributes())
    {
        if (attr.name == "POSITION_CARTESIAN")
        {
            for (std::size_t i = 0; i < point_count; ++i)
            {
                ...
                float x = *reinterpret_cast<std::uint32_t*>(&data[index + 0]) * meta_data_->scale_[0] + translate.x;
                float y = *reinterpret_cast<std::uint32_t*>(&data[index + 4]) * meta_data_->scale_[1] + translate.y;
                float z = *reinterpret_cast<std::uint32_t*>(&data[index + 8]) * meta_data_->scale_[2] + translate.z;
        ...
        else if (attr.name == "COLOR_PACKED" || attr.name == "RGBA")
        {
            ...
            {
                std::size_t index = offset + i * meta_data_->point_byte_size_;
                float r = 1.f * static_cast<unsigned char>(data[index + 0]) / 255.f;
                float g = 1.f * static_cast<unsigned char>(data[index + 1]) / 255.f;
                float b = 1.f * static_cast<unsigned char>(data[index + 2]) / 255.f;
                float a = 1.f * static_cast<unsigned char>(data[index + 3]) / 255.f;
...
fs::path CloudLoader1::fileName(const std::shared_ptr<CloudMetaData>& meta_data, const std::string& name,
